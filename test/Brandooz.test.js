...
contract("Brandooz", async function(accounts) {
  let token, metadata
  ...
  before(done => {
    ;(async () => {
      try {
        ...
        // Deploy Brandooz.sol
        token = await Brandooz.new("Brandooz", "BDZ", metadata.address)
        var tx = await web3.eth.getTransactionReceipt(token.transactionHash)
        totalGas = totalGas.plus(tx.gasUsed)
        console.log(_ + tx.gasUsed + " - Deploy Brandooz")
        token = await Brandooz.deployed()
  ...
  it("should mint a token from the owner account", async function() {
    ...
    assert(
      ...
      "Contract should have balance of 1 instead it has " +
    ...
    assert(
      ...
      "Owner account should have 1 token instead it has " +
